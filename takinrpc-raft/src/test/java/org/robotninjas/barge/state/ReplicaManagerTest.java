...
public class ReplicaManagerTest {
    ...
    public void testInitialSendOutstanding() {
        ...
        AppendEntries appendEntries = AppendEntries.newBuilder().setLeaderId(SELF.toString()).setCommitIndex(0).setPrevLogIndex(0).setPrevLogTerm(0).setTerm(1).build();
    ...
    public void testFailedAppend() {
        ...
        when(mockClient.appendEntries(eq(FOLLOWER), any(AppendEntries.class))).thenReturn(response).thenReturn(mock(ListenableFuture.class));
        ...
        AppendEntriesResponse appendEntriesResponse = AppendEntriesResponse.newBuilder().setLastLogIndex(0L).setTerm(1).setSuccess(false).build();
    ...
    public void testSuccessfulAppend() {
        ...
        Entry entry = Entry.newBuilder().setTerm(1).setCommand(ByteString.EMPTY).build();
        ...
        AppendEntries appendEntries = AppendEntries.newBuilder().setLeaderId(SELF.toString()).setCommitIndex(0).setPrevLogIndex(0).setPrevLogTerm(0).setTerm(1).addEntries(entry).build();

        assertTrue(replicaManager.isRunning());
        assertFalse(replicaManager.isRequested());
        assertEquals(1, replicaManager.getNextIndex());

        AppendEntriesResponse appendEntriesResponse = AppendEntriesResponse.newBuilder().setLastLogIndex(0L).setTerm(1).setSuccess(true).build();
